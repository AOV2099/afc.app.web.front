<script>
	...
	import { Home, Calendar, History as HistoryIcon, User } from 'lucide-svelte';
	...
	let NAV = [
		{ key: 'home', label: 'Inicio', path: '/app/home', icon: Home, active: false },
		{ key: 'explore', label: 'Explorar', path: '/app/explore', icon: Calendar, active: false },
		{ key: 'history', label: 'Historial', path: '/app/history', icon: HistoryIcon, active: false },
		{ key: 'account', label: 'Perfil', path: '/app/account', icon: User, active: false }
	];
	...
	<aside class="fixed inset-y-0 left-0 hidden w-64 border-r bg-background sm:flex">
		<div class="flex w-full flex-col p-4">
			<div class="flex items-center justify-between">
				...
				<Button variant="ghost" size="icon" class="rounded-full" on:click={() => goTo('account')}>
